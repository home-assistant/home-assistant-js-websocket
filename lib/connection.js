...
function getSocket(auth, options) {
      ...
      switch (message.type) {
        case MSG_TYPE_AUTH_REQUIRED:
          try {
            ...
            socket.send(JSON.stringify(messages.authAccessToken(auth.access_token)));
          } catch (err) {
...
class Connection {
  ...
  async subscribeEvents(eventCallback, eventType) {
    const resultMessage = await this.sendMessagePromise(messages.subscribeEvents(eventType));
    ...
    const info = {
      ...
      eventType,
      unsubscribe: async () => {
        await this.sendMessagePromise(messages.unsubscribeEvents(resultMessage.id));
        delete this.commands[resultMessage.id];
      },
    };
...
export default function createConnection(auth, options = {}) {
  return getSocket(auth, options)
    .then((socket) => {
